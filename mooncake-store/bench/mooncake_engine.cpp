...
bool MooncakeEngine::Init() {
    ...
    // Create a new client for this instance
    int port = getRandomAvailablePort();
    std::string local_hostname = "localhost:" + std::to_string(port);
    client_ = new mooncake::Client();
    void **args = rdma_args("ibp51s0");
    ErrorCode rc = client_->Init(local_hostname, "localhost:2379", "rdma", args,
                                 "127.0.0.1:50051");
    if (rc != ErrorCode::OK) {
    ...
    rc = client_->MountSegment(local_hostname, ptr, kSegmentSize);
    if (rc != ErrorCode::OK) {
    ...
    rc = client_->RegisterLocalMemory(client_buffer_allocator_->getBase(),
                                      kLocalBufferSize, "cpu:0", false, false);
    if (rc != ErrorCode::OK) {
